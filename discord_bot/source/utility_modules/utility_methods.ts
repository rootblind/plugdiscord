...
import { Auth, ServerURL } from "../server/interfaces/server_types.js";
...
/**
 * 
 * @param url The ServerURL object
...
export async function pingServerConnection(
    ...
    try {
        const response = await axios.get(
            `${url.host}:${url.port}/${url.route}/${url.endpoint}`,
            {
                timeout: 3_000,
                params: {
                    GuildId: guildId
        ...
        return response.data.ServerName;
    } catch {
        return false;
    }
}