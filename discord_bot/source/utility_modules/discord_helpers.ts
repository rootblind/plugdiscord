 ...
 * 
 * @param guild Guild object
 * @returns Array of Hycord channels
 */
export async function fetchHycordGuildChannels(guild: Guild): Promise <BaseGuildTextChannel[]> {
    const channelIds = await HycordChannelRepo.getGuildChannels(guild.id);
    const channels: BaseGuildTextChannel[] = [];
    if(channelIds) {
